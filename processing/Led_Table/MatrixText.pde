public class MatrixText {
  ...
  /* Takes in a character and returns the 5*8 bytes for that character */
  ...
  void drawChar(char c, int size, int xCoord, int yCoord) {
    ...
    for (byte i = 0; i < 8; ++i) {
      for (byte j = 0; j < 5; ++j) {
        ...
        //println("index:", index, "x:", x, "y:", y, "val:", val);
        if (val>0){
  ...
  void drawString(String input, int size, int x, int y) {
    int strLen = input.length();
    for (int i = 0; i < strLen; ++i) {
      drawChar(input.charAt(i), size, x+ (size*6*i), y); 
    }
  }
